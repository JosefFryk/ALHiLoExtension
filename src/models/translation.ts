...
const translateText = async (
  ...
  const initialResult = await callAITranslation(text, sourceLang, targetLang, numOptions, apiKey, apiEndpoint, deployment, apiVersion);
 ...
 const examples = await lookupFuzzyExamples(text, sourceLang);
  ...
  return await callAITranslation(text, sourceLang, targetLang, numOptions, apiKey, apiEndpoint, deployment, apiVersion, enrichedPrompt);
};
...
async function callAITranslation(
  ...
  try {
    ...
    const results = (response.data.choices as { message?: { content: string } }[]).map((choice) => {
      ...
      const probabilities = tokenLogProbs.map((t: any) =>
      ...
      const combinedConfidence = parseFloat(((avgProb * 0.7) + (minProb * 0.3)).toFixed(2)); // test minProb instead of avgProb
    ...
    if (errorMessage.includes('model loading')) {
      ...
      return callAITranslation(text, sourceLang, targetLang, numOptions, apiKey, endpoint, deployment, apiVersion, examples);
    }
